      ...
      if (functionObj.events) {
        functionObj.events.forEach((event, idx) => {
          if (typeof event.eventBridge === 'object') {
            ...
            let ruleResource = `arn:aws:events:*:*:rule/${RuleName}`;
            if (EventBus) {
              let eventBusName = EventBus;
              if (EventBus.startsWith('arn')) {
                eventBusName = EventBus.split('/').pop();
              }
              eventBusResource = `arn:aws:events:*:*:event-bus/${eventBusName}`;
              ruleResource = `arn:aws:events:*:*:rule/${eventBusName}/${RuleName}`;
